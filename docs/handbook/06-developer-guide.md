...
metaphor migration generate Invoice deal
metaphor migration run
...
(`deal` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed deal          # run Rust seeders
metaphor migration generate-seeds deal  # emit SQL seed files
```
